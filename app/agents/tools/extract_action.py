...
extract_action_json = {
            "type": "function",
            "name": "extract_action",
            "description": "Извлечь ТОЛЬКО элемент Action (Действия) из ПРЕДОСТАВЛЕННОЙ стенограммы собеседования. НЕ ДОБАВЛЯЙТЕ никакой информации, которой нет в тексте. НЕ ВЫДУМЫВАЙТЕ. Используйте только факты из стенограммы.",
            "parameters": {
                "type": "object",
                "properties": {
                    "Action": {"type": "string", "description": "Извлеченные ДОСЛОВНО действия из собеседования. Если действия не описаны ЯВНО, укажите 'Действия не описаны'. Не делайте выводов и не обобщайте."}
                },
                "required": ["Action"],
...
def extract_action(messages: List[Message]) -> str:
    """
    Извлечь элемент Action (Действия) из стенограммы собеседования. НЕ ВЫДУМЫВАТЬ. Использовать ТОЛЬКО текст стенограммы.

    Args:
        messages (list): Список сообщений из стенограммы собеседования. Каждое сообщение - словарь, содержащий 'role' и 'content'.

    Return:
        str: Извлеченные ДОСЛОВНО действия. Если действия не описаны ЯВНО - 'Действия не описаны'.
    """
    ...
    # Используем отфильтрованные сообщения для извлечения действий
    ...
    extraction_messages = [
        {"role": "system", "content": "Ты должен извлечь ТОЛЬКО Действия (Action) из предоставленного текста. Не добавляй ничего от себя. Не интерпретируй. Если Действия не описаны явно, ВЕРНИ 'Действия не описаны'."}
    ]
    ...
    if isinstance(response, dict) and "Action" in response:
        extracted_action = response["Action"]
        if not extracted_action or extracted_action.strip() == "" or "не описаны" in extracted_action.lower():
             ...
             if not combined_filtered_text.strip(): # Если и отфильтрованный текст пуст
                 ...
                 return extracted_action # Возвращаем ответ модели (вероятно, 'Действия не описаны')
