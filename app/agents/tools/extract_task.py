...
extract_task_json = {
            "type": "function",
            "name": "extract_task",
            "description": "Извлечь ТОЛЬКО элемент Task (Задача) из ПРЕДОСТАВЛЕННОЙ стенограммы собеседования. НЕ ДОБАВЛЯЙТЕ никакой информации, которой нет в тексте. НЕ ВЫДУМЫВАЙТЕ. Используйте только факты из стенограммы.",
            "parameters": {
                "type": "object",
                "properties": {
                    "Task": {"type": "string", "description": "Извлеченная ДОСЛОВНО задача из собеседования. Если задача не описана ЯВНО, укажите 'Задача не описана'. Не делайте выводов и не обобщайте."}
                },
                "required": ["Task"],
...
def extract_task(messages: List[Message]) -> str:
    """
    Извлечь элемент Task (Задача) из стенограммы собеседования. НЕ ВЫДУМЫВАТЬ. Использовать ТОЛЬКО текст стенограммы.

    Args:
        messages (list): Список сообщений из стенограммы собеседования. Каждое сообщение - словарь, содержащий 'role' и 'content'.

    Return:
        str: Извлеченная ДОСЛОВНО задача. Если задача не описана ЯВНО - 'Задача не описана'.
    """
    ...
    # Используем отфильтрованные сообщения для извлечения задачи
    ...
    extraction_messages = [
        {"role": "system", "content": "Ты должен извлечь ТОЛЬКО Задачу (Task) из предоставленного текста. Не добавляй ничего от себя. Не интерпретируй. Если Задача не описана явно, ВЕРНИ 'Задача не описана'."}
    ]
    ...
    if isinstance(response, dict) and "Task" in response:
        extracted_task = response["Task"]
        if not extracted_task or extracted_task.strip() == "" or "не описана" in extracted_task.lower():
             ...
             if not combined_filtered_text.strip(): # Если и отфильтрованный текст пуст
                 ...
                 return extracted_task # Возвращаем ответ модели (вероятно, 'Задача не описана')
